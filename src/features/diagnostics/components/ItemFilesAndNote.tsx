...
export function ItemFilesAndNote({
  item,
  order,
  onDeleteFile,
  isDeletingFile,
          ...
          {item.files.map((f) => (
            <div key={f.id} className="flex items-center gap-1.5 text-xs border border-border rounded-lg px-2 py-1 bg-surface-hover hover:border-border-strong transition-colors">
              ...
              {item.status !== "CANCELLED" && (
                <button onClick={() => onDeleteFile(f.id)} disabled={isDeletingFile} className="text-text-muted hover:text-danger transition-colors disabled:opacity-40 ml-0.5" aria-label={t("diagnostics.deleteFile")}>
