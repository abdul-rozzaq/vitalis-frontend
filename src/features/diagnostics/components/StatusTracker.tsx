...
import { TRACKER_STEPS } from "../constants/status-styles";
import { DiagnosticItemStatus, DiagnosticOrderItem } from "../types";

import { formatDateTime } from "@/shared/lib/formatters";

export function StatusTracker({ item }: { item: DiagnosticOrderItem }) {
  ...
  const currentIdx = TRACKER_STEPS.indexOf(item.status as Exclude<DiagnosticItemStatus, "CANCELLED">);
        ...
        return (
          <div key={step} className="flex flex-col items-center flex-1">
            ...
            <div className="mt-1.5 text-center">
              <p className={`text-[9px] font-semibold leading-tight tracking-wide ${isDone ? "text-success" : isActive ? "text-blue-700" : "text-text-muted"}`}>{t(`diagnostics.itemStatus.${step}`)}</p>
