...
import { RevenueByDepartmentRow } from "../chart-colors";

interface Props {
  rows: RevenueByDepartmentRow[];
}

export function RevenueDepartmentTable({ rows }: Props) {
  ...
  return (
    <div className="bg-surface border border-border rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-border">
        ...
        <p className="text-[10px] text-text-muted mt-0.5">{t("invoices.reports.byDepartmentNote")}</p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-text-muted border-b border-border">
                <th className="text-left font-medium px-4 py-2">{t("invoices.reports.table.department")}</th>
            ...
            <tbody>
              {sorted.map((row) => {
                ...
                return (
                  <tr key={row.departmentId ?? "none"} className="border-b border-border last:border-0">
                    <td className="px-4 py-3 font-medium text-text whitespace-nowrap">
                      {row.departmentId ? row.departmentName : <span className="text-text-muted italic">{row.departmentName}</span>}
                    </td>
