...
import {
  Building2, CheckCircle2, CreditCard, Download, Edit,
  Loader2, Plus, SlidersHorizontal, Trash2, TrendingUp, Users, X,
} from "lucide-react";
...
const INITIAL_FILTERS: PaymentFilters = {
  status: "", minAmount: "", maxAmount: "", dateFrom: "", dateTo: "", departmentId: "",
};
...
const MOCK_DEPARTMENTS: DepartmentOption[] = [
  { id: "d1", name: "Cardiology" }, { id: "d2", name: "Neurology" },
  { id: "d3", name: "Pediatrics" }, { id: "d4", name: "Orthopedics" },
  { id: "d5", name: "Emergency" }, { id: "d6", name: "Radiology" },
];

const MOCK_PAYMENTS: Payment[] = [
  { id: "pay-001", patient_id: "p1", patient_name: "Alice Johnson", department_id: "d1", department_name: "Cardiology", amount: 450.0, method: "CREDIT_CARD", status: "PAID", description: "Routine cardiac checkup", createdAt: "2026-02-20T10:30:00.000Z" },
  { id: "pay-002", patient_id: "p2", patient_name: "Bob Smith", department_id: "d5", department_name: "Emergency", amount: 1200.0, method: "CASH", status: "UNPAID", description: "Emergency room visit", createdAt: "2026-02-21T14:15:00.000Z" },
  { id: "pay-003", patient_id: "p3", patient_name: "Carol Williams", department_id: "d3", department_name: "Pediatrics", amount: 280.5, method: "CASH", status: "PAID", description: "Pediatric consultation", createdAt: "2026-02-22T09:00:00.000Z" },
  { id: "pay-005", patient_id: "p5", patient_name: "Eva Martinez", department_id: "d6", department_name: "Radiology", amount: 320.0, method: "CREDIT_CARD", status: "UNPAID", description: "MRI scan", createdAt: "2026-02-24T16:00:00.000Z" },
];
...
function FilterPanel({ filters, onChange, onReset, departments, activeCount }: {
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm font-medium text-text flex items-center gap-2">
          ...
          {t("common.filter")}
          {activeCount > 0 && (
            <span className="bg-primary text-white text-[10px] font-semibold px-1.5 py-0.5 rounded-full leading-none">{activeCount}</span>
          )}
        </span>
        {activeCount > 0 && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        ...
        <div className="space-y-1">
          ...
          <input type="number" min="0" value={filters.minAmount} onChange={(e) => onChange("minAmount", e.target.value)} placeholder="0"
        ...
        <div className="space-y-1">
          ...
          <input type="number" min="0" value={filters.maxAmount} onChange={(e) => onChange("maxAmount", e.target.value)} placeholder="∞"
            className="w-full bg-surface border border-border rounded-md px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent/20 focus:border-accent transition-all" />
        </div>
        <div className="space-y-1">
        ...
        <div className="space-y-1 col-span-2 sm:col-span-3 lg:col-span-5">
          ...
          <select value={filters.departmentId} onChange={(e) => onChange("departmentId", e.target.value)}
...
export default function PaymentsPage() {
  ...
  const { mutateAsync: updatePayment } = useMutation({
    ...
    onError: (_err, _vars, context) => {
      if (context?.previous)
        queryClient.setQueryData(["payments", filters], context.previous);
    },
    onSettled: () => {
  ...
  const payments: Payment[] = useMemo(
    () => Array.isArray(paymentsRaw) ? paymentsRaw.map(normalizePayment) : MOCK_PAYMENTS,
    [paymentsRaw]
  );
  ...
  const handleExport = () => {
    ...
    const rows = filteredPayments.map((p: Payment) => [p.patient_name ?? "", p.department_name ?? "", p.amount, METHOD_LABELS[p.method] ?? p.method, STATUS_LABELS[p.status] ?? p.status, p.createdAt ? new Date(p.createdAt).toLocaleDateString() : ""]);
    exportToExcel("payments", headers, rows, t("payments.title"));
  };

  const handleAdd = () => { setEditingPayment(null); setIsSheetOpen(true); };
  const handleEdit = (p: Payment) => { setEditingPayment(p); setIsSheetOpen(true); };
  const handleDelete = (id: string) => {
    if (confirm(t("payments.deleteConfirm"))) { setDeletingId(id); deletePayment(id); }
  };
  ...
  const handleSubmit = (data: any) => {
    ...
    const action = editingPayment
      ? updatePayment({ id: editingPayment.id, data: payload })
      : addPayment(payload);
    action.then(() => { setIsSheetOpen(false); setEditingPayment(null); });
  };
  ...
  const columns = useMemo<ColumnDef<Payment>[]>(() => [
    ...
    {
      accessorKey: "patient_name", header: t("payments.colPatient"),
      cell: ({ row }) => {
    ...
    {
      accessorKey: "department_name", header: t("payments.colDepartment"),
      cell: ({ row }) => (
        <div className="flex items-center gap-1.5 text-secondary text-sm">
          <Building2 className="w-3.5 h-3.5 shrink-0" />{row.original.department_name || "—"}
        </div>
      ),
    },
    {
      accessorKey: "amount", header: t("payments.colAmount"),
      cell: (info: any) => (
        <span className="font-semibold text-text text-sm">{Number(info.getValue()).toLocaleString("uz-UZ")} so'm</span>
      ),
    },
    {
      accessorKey: "method", header: t("payments.colMethod"),
      cell: (info: any) => {
        ...
        return (
          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${s.bg} ${s.text}`}>
            <CreditCard className="w-3 h-3" />{METHOD_LABELS[method] ?? method}
          </span>
        );
      },
    },
    {
      accessorKey: "status", header: t("payments.colStatus"),
      cell: (info: any) => {
    ...
    {
      accessorKey: "createdAt", header: t("payments.colDate"),
      cell: (info: any) => (
          ...
          <div className="flex justify-end items-center gap-1">
            ...
            <Can roles={["ADMIN", "KASSIR", "HISOBCHI"]}>
              <button onClick={() => handleEdit(payment)}
                className="p-1.5 rounded-md hover:bg-surface-hover text-secondary transition-colors cursor-pointer"
  ...
  return (
    <div className="p-6 space-y-5 max-w-6xl mx-auto w-full">
      ...
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setFilterOpen((v) => !v)}
            ...
            {t("common.filter")}
            {activeFilterCount > 0 && (
              <span className="bg-primary text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full leading-none ml-0.5">{activeFilterCount}</span>
            )}
          </button>
          ...
          <Can roles={["ADMIN", "KASSIR"]}>
            <button onClick={handleAdd}
              ...
              <Plus className="w-3.5 h-3.5" />{t("payments.newPayment")}
            </button>
          </Can>
        </div>
      </motion.div>
      ...
      <AnimatePresence>
        {filterOpen && (
          <FilterPanel filters={filters} onChange={handleFilterChange}
            onReset={handleFilterReset} departments={departments} activeCount={activeFilterCount} />
        )}
      </AnimatePresence>
      ...
      <QuickPayDialog
        ...
        initialAmount={quickPayTarget?.amount ?? 0}
        patientName={quickPayTarget?.patient_name}
      />
      ...
      <Sheet isOpen={isSheetOpen} onClose={() => setIsSheetOpen(false)}
        ...
        <PaymentForm
          patients={patients} departments={departments}
          initialData={editingPayment ? {
            patientId: editingPayment.patient_id, departmentId: editingPayment.department_id,
            amount: editingPayment.amount, method: editingPayment.method,
            status: editingPayment.status, description: editingPayment.description,
          } : undefined}
          onSubmit={handleSubmit} onCancel={() => setIsSheetOpen(false)}
        />
      </Sheet>
    </div>
  );
