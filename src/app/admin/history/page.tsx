...
const EVENT_TYPES: ShiftEventType[] = ["FULL_TRANSFER", "PARTIAL_TRANSFER", "OVERTIME_ADDED", "OVERRIDE_CREATED", "OVERRIDE_DELETED", "SWAP_REQUEST", "SWAP_APPROVED", "SWAP_REJECTED"];
              ...
              <tbody>
                {events.map((e) => (
                  <tr key={e.id} className="border-b border-border last:border-0 hover:bg-surface-hover">
                    ...
                    <td className="p-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full border ${SHIFT_EVENT_COLOR[e.eventType]}`}>{SHIFT_EVENT_LABEL[e.eventType]}</span>
                    </td>
                    <td className="p-3 text-text">{e.roomShift?.name ?? "—"}</td>
                    <td className="p-3 text-text">
                      {e.fromDoctor.first_name} {e.fromDoctor.last_name}
                    </td>
